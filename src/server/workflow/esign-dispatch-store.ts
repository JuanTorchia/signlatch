...
export class ESignDispatchStore {
  ...
  async releaseSafeRetry(lease:{dispatch_id:string;provider_operation_id:string;leasedBy:string;lease_generation:number},errorCode:string){await this.sql.begin(async tx=>{const result=await tx`update esign_dispatches set status='pending',leased_by=null,lease_expires_at=null,updated_at=now() where dispatch_id=${lease.dispatch_id} and status='processing' and leased_by=${lease.leasedBy} and lease_generation=${lease.lease_generation}`;if(result.count!==1)throw new Error("Dispatch lease lost");const operation=await tx`update provider_operations set state='reserved',result_payload=${tx.json({errorCode})},leased_by=null,lease_expires_at=null,updated_at=now() where operation_id=${lease.provider_operation_id} and state='running' and leased_by=${lease.leasedBy}`;if(operation.count!==1)throw new Error("Provider budget lease lost");});}
  async markDenied(lease:{dispatch_id:string;workflow_id:string;provider_operation_id:string;leasedBy:string;lease_generation:number},errorCode:string){await this.sql.begin(async tx=>{const result=await tx`update esign_dispatches set status='denied',leased_by=null,lease_expires_at=null,updated_at=now() where dispatch_id=${lease.dispatch_id} and status='processing' and leased_by=${lease.leasedBy} and lease_generation=${lease.lease_generation}`;if(result.count!==1)throw new Error("Dispatch lease lost");const operations=await tx<Array<{tenant_id:string;operation_kind:string;reserved_units:number;created_at:Date}>>`update provider_operations set state='failed',result_payload=${tx.json({errorCode})},leased_by=null,lease_expires_at=null,updated_at=now() where operation_id=${lease.provider_operation_id} and state='running' and leased_by=${lease.leasedBy} returning tenant_id,operation_kind,reserved_units,created_at`;const operation=operations[0];if(!operation)throw new Error("Provider budget lease lost");const periodStart=new Date(Date.UTC(operation.created_at.getUTCFullYear(),operation.created_at.getUTCMonth(),operation.created_at.getUTCDate()));await tx`update provider_budgets set reserved=reserved-${operation.reserved_units},version=version+1 where tenant_id=${operation.tenant_id} and provider='foxit' and operation_kind=${operation.operation_kind} and period_start=${periodStart}`;await tx`update agreement_workflows set state='failed',updated_at=now() where workflow_id=${lease.workflow_id} and state='dispatching'`;});}
