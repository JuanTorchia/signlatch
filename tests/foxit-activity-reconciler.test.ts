...
test("maps authenticated Foxit activity into deterministic sanitized lifecycle events", () => {
  ...
  const payload = { details: { activities: [
    { action: "Created", folderStatus: "CREATED", time: "28 Aug 2026, 00:03:48, PDT" },
    ...
    { action: "Viewed", folderStatus: "EXECUTED", time: "28 Aug 2026, 00:16:00, PDT" },
  ...
  assert.equal(first[0].type, "created");
  assert.deepEqual(new Set(first.slice(1).map((event) => event.type)), new Set(["sent", "viewed", "signed", "executed"]));
